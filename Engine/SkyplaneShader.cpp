...
bool SkyplaneShader::InitStandard(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	device->CreateBuffer(&skyBufferDesc, NULL, &m_skyBuffer);
	return true;
}

bool SkyplaneShader::SetShaderParameters(
...
)
{
	...
	context->Map(m_skyBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	...
	context->Unmap(m_skyBuffer, 0);
	context->PSSetConstantBuffers(0, 1, &m_skyBuffer);	//note the first variable is the mapped buffer ID.  Corresponding to what you set in the PS

	//pass the desired texture to the pixel shader.
	context->PSSetShaderResources(0, 1, &cloudTexture);
	context->PSSetShaderResources(1, 1, &perturbTexture);

	return false;
}

void SkyplaneShader::EnableShader(ID3D11DeviceContext* context)
{
